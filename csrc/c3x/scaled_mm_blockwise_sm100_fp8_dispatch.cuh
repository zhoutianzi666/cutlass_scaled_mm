...
void cutlass_gemm_caller_blockwise(paddle::Tensor& out, paddle::Tensor const& a,
  ...
  auto a_scales_ptr = static_cast<ElementBlockScale const*>(const_cast<void*>(a_scales.data()));
  auto b_scales_ptr = static_cast<ElementBlockScale const*>(const_cast<void*>(b_scales.data()));
...
void cutlass_gemm_blockwise_sm100_fp8_dispatch(paddle::Tensor& out,
                                               ...
                                               paddle::Tensor const& a_scales,
                                               paddle::Tensor const& b_scales) {
  int32_t m = a.shape()[0], n = b.shape()[0], k = a.shape()[1], sms;
  cudaDeviceGetAttribute(&sms, cudaDevAttrMultiProcessorCount, a.place().GetDeviceId());
